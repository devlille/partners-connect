...
import { getCompanies, getCompaniesPartnership, getCompaniesJobOffers, postCompaniesJobOffers, deleteCompaniesJobOffersById, type JobOfferResponseSchema, type CreateJobOfferSchema } from '~/utils/api';
...
// Charger le company_id à partir du sponsorId
async function loadCompanyId() {
  try {
    // On doit trouver quelle compagnie correspond à ce partenariat
    // Pour cela, on va chercher tous les partenariats de toutes les compagnies
    // et trouver celui qui correspond au sponsorId

    // Note: Cette approche n'est pas optimale, l'idéal serait que l'API
    // retourne le company_id dans le PartnershipItemSchema
    // Pour l'instant, on va devoir faire une recherche

    // Charger toutes les compagnies et chercher le partenariat
    const companiesResponse = await getCompanies();
    const companies = companiesResponse.data.items;

    for (const company of companies) {
      try {
        const partnershipsResponse = await getCompaniesPartnership(company.id);
        const partnership = partnershipsResponse.data.find((p: any) => p.id === sponsorId.value);
