import {
  ...
  getEventsPartnershipBilling,
  putCompanyById,
  putEventsPartnershipBilling,
} from "~/utils/api";
...
export const usePublicPartnership = () => {
  ...
  async function loadPartnership() {
    try {
      ...
      // Load billing data
      try {
        const billingResponse = await getEventsPartnershipBilling(
          eventSlug.value,
          partnershipId.value,
        );
      ...
      const packOptions = (
        (p.selected_pack as any)?.options ||
        p.selected_pack?.optional_options ||
        []
      ).map((opt: any) => ({
        id: opt.id,
        name: opt.name,
      ...
      partnership.value = {
        ...
        validated:
          p.process_status?.validated_at !== null && p.process_status?.validated_at !== undefined,
        paid: p.process_status?.billing_status?.toLowerCase() === "paid",
        suggestion: false,
        agreement_generated:
          p.process_status?.agreement_url !== null && p.process_status?.agreement_url !== undefined,
        agreement_signed:
          p.process_status?.agreement_signed_url !== null &&
          p.process_status?.agreement_signed_url !== undefined,
      ...
      if (err.response?.status === 404) {
        ...
        error.value =
          "Network Error: No internet connection detected. Please check your network connection and refresh the page to retry.";
