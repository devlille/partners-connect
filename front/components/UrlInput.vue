...
import { URL_REGEX, VALIDATION_MESSAGES } from '~/constants/validation';
...
const inputId = computed(() => `url-${Math.random().toString(36).substring(2, 9)}`);
...
const errorMessage = computed(() => {
  ...
  if (value) {
    ...
    // Si allowHttp est false, vérifier que c'est HTTPS
    if (!props.allowHttp && value.startsWith('http://')) {
      return 'URL doit utiliser HTTPS (https://)';
    }
  }
...
const isValid = computed(() => {
  ...
  if (value) {
    ...
    if (!props.allowHttp && value.startsWith('http://')) return false;
  }
