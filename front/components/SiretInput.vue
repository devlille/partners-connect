...
import { SIRET_REGEX, VALIDATION_MESSAGES } from '~/constants/validation';
...
const inputId = computed(() => `siret-${Math.random().toString(36).substring(2, 9)}`);
...
const errorMessage = computed(() => {
  ...
  if (value && !SIRET_REGEX.test(value)) {
    return VALIDATION_MESSAGES.SIRET_INVALID;
  }
...
const isValid = computed(() => {
  ...
  if (value && !SIRET_REGEX.test(value)) return false;
