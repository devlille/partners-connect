<template>
  <div class="space-y-4">
    ...
    <!-- Logo actuel -->
    <div v-if="currentLogo && !preview" class="relative inline-block">
      <div
...
function validateAndPreviewFile(file: File) {
  ...
  if (!validTypes.includes(file.type)) {
    toast.add({ title: 'Format non supporté', description: 'Veuillez sélectionner un fichier SVG, PNG ou JPEG', color: 'error' });
    return;
  }

  const maxSize = 5 * 1024 * 1024;
  if (file.size > maxSize) {
    toast.add({ title: 'Fichier trop volumineux', description: 'Le logo ne doit pas dépasser 5MB', color: 'error' });
    return;
  }
...
async function handleUpload() {
  ...
  try {
    const media = await postCompaniesLogo(props.companyId, { file: selectedFile.value });

    toast.add({ title: 'Succès', description: 'Le logo a été mis à jour avec succès', color: 'success' });
