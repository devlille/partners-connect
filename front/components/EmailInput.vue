...
const inputId = computed(() => `email-${Math.random().toString(36).substring(2, 9)}`);
...
function handleInput(event: Event) {
  ...
  // Convertir en minuscules (convention standard pour les emails)
  const normalizedValue = value.toLowerCase().trim();

  internalValue.value = normalizedValue;
  emit('update:modelValue', normalizedValue || null);
