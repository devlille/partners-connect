...
import { VAT_REGEX, VALIDATION_MESSAGES } from '~/constants/validation';
...
const inputId = computed(() => `vat-${Math.random().toString(36).substring(2, 9)}`);
...
const errorMessage = computed(() => {
  ...
  if (value && !VAT_REGEX.test(value)) {
    return VALIDATION_MESSAGES.VAT_INVALID;
  }
...
const isValid = computed(() => {
  ...
  if (value && !VAT_REGEX.test(value)) return false;
