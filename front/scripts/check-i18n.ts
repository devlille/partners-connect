...
function findHardcodedTexts(dirPath: string, extensions = ['.vue', '.ts']): Map<string, string[]> {
  ...
  function scanFile(filePath: string) {
    ...
    if (filePath.includes('node_modules') ||
        ...
        filePath.includes('utils/api.ts')) { // Fichier généré
      return
    }
...
function generateReport() {
  ...
  for (const locale of locales) {
    ...
    console.log(`${colors.green}✓${colors.reset} Chargé ${locale}: ${Object.keys(translations[locale]).length} clés`)
  }
  ...
  for (const locale of locales) {
    ...
    const { missing, extra } = compareTranslations(
      'fr-FR',
      frKeys,
      locale,
      translations[locale]
  ...
  for (const dir of directories) {
    ...
    if (hardcoded.size > 0) {
      hasIssues = true
      console.log(`${colors.yellow}⚠${colors.reset}  Textes hardcodés trouvés dans ${path.basename(dir)}/:`)
  ...
  if (totalHardcoded > 0) {
    console.log(`${colors.red}✗${colors.reset} Total: ${totalHardcoded} textes hardcodés détectés\n`)
  } else {
