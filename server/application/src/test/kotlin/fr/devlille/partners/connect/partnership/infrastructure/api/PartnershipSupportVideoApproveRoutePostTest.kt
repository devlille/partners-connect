...
class PartnershipSupportVideoApproveRoutePostTest {
    ...
    @Test
    fun `approve returns 404 when video belongs to a different partnership`() = testApplication {
        val userId = UUID.randomUUID()
        val orgId = UUID.randomUUID()
        val eventId = UUID.randomUUID()
        val packId = UUID.randomUUID()
        ...
        val response = client.post(
            "/orgs/$orgId/events/$eventId/partnerships/$partnershipBId/support-videos/$videoId/approve",
        ) {
            header(HttpHeaders.Authorization, "Bearer valid")
            contentType(ContentType.Application.Json)
        }
    ...
    @Test
    fun `approve returns 404 when video belongs to a different event`() = testApplication {
        val userId = UUID.randomUUID()
        val orgId = UUID.randomUUID()
        ...
        val response = client.post(
            "/orgs/$orgId/events/$eventBSlug/partnerships/$partnershipId/support-videos/$videoId/approve",
        ) {
            header(HttpHeaders.Authorization, "Bearer valid")
            contentType(ContentType.Application.Json)
        }
